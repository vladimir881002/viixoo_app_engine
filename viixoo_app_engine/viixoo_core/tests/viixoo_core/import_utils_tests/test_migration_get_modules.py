...
class MockBaseDBModel(BaseDBModel):
    """Mock BaseDBModel class."""

    __tablename__ = "mock_table"


class TestMigrationGetModules:
    """Test migration get_modules function."""
    ...
    def test_get_modules_custom_app_path(
        self, mock_dirname, mock_join, mock_iter_modules, capsys, monkeypatch
    ):
        """Test get_modules with a custom APPS_PATH environment variable."""
        monkeypatch.setenv("APPS_PATH", "/custom/app/path")

        mock_iter_modules.return_value = [(None, "module1", False)]
