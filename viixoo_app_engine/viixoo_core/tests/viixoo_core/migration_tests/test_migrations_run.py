...
class TestMigrationRun:
    ...
    def test_run_postgresql(self, mock_run_postgresql_migrations, mock_get_config, mock_import_module, capsys):
        """Test run method with PostgreSQL database."""
        # Arrange
        mock_import_module.return_value = {'module1': MagicMock(), 'module2': MagicMock()}
        mock_get_config.side_effect = [
            {"db_type": "postgresql", "user": "user1", "password": "password1", "host": "host1", "port": "port1", "dbname": "dbname1"},
            {"db_type": "postgresql", "user": "user2", "password": "password2", "host": "host2", "port": "port2", "dbname": "dbname2"}
        ]
    ...
    @patch.object(BaseConfig, "get_config")
    @patch.object(Migration, "run_postgresql_migrations")
    def test_run_module_exception(self, mock_run_postgresql_migrations, mock_get_config, mock_import_module):
        """Test run method when a module raise an exception."""
        # Arrange
        mock_import_module.side_effect = Exception("Some error")
        mock_get_config.return_value = {"db_type": "postgresql", "user": "user1", "password": "password1", "host": "host1", "port": "port1", "dbname": "dbname1"}
    ...
    @patch.object(BaseConfig, "get_config")
    @patch.object(Migration, "run_postgresql_migrations")
    def test_run_no_modules(self, mock_run_postgresql_migrations, mock_get_config, mock_import_module, capsys):
