...
class Migration:
    ...
    def get_postgresql_connection(cls):
        """Gets the connection to PostgreSQL."""        
        ...
        if not db_exists:
            ...
            cursor.execute(SQL("CREATE DATABASE {}").format(Identifier(config["dbname"])))
    ...
    def create_migration_log_tables(cls, cursor):
        """Creates log tables if they do not exist."""
        cursor.execute("""
    ...
    def log_change(cls, action: str, description: str):
        """Logs a change in the database structure."""
        conn = cls.get_postgresql_connection()
        cursor = conn.cursor()
        cursor.execute("""
    ...
    def table_exists(cls, cursor, table_name: str) -> bool:
        """Checks if the table already exists in PostgreSQL."""
        cursor.execute("""
            SELECT EXISTS (
    ...
    def get_existing_columns(cls, cursor, table_name: str) -> dict:
        """Gets the existing columns in a table with their types and constraints."""
        cursor.execute(f"""
    ...
    @classmethod
    def add_foreign_key(cls, cursor, table_name: str, column: str, foreign_table: str, on_delete: str, on_update: str):
        """
            Adds a foreign key with configurable ON DELETE and ON UPDATE policies.
            Supported values:
        ...
        cls.log_change("ADD FOREIGN KEY", f"Foreign key '{column}' -> '{foreign_table}.id' in '{table_name}' with ON DELETE {on_delete} and ON UPDATE {on_update}")

    @classmethod
    def remove_foreign_key(cls, cursor, table_name: str, column: str):
        """Removes a foreign key if it is no longer in the model."""
        fk_name = f"{table_name}_{column}_fk"
        cursor.execute(f"ALTER TABLE {table_name} DROP CONSTRAINT IF EXISTS {fk_name};")
        cls.log_change("REMOVE FOREIGN KEY", f"FK '{column}' removed from '{table_name}'")

    @classmethod
    def get_existing_foreign_keys(cls, cursor, table_name: str):
        """Returns a dictionary with the existing foreign keys in the table."""
        cursor.execute(f"""
    ...
    @classmethod
    def remove_obsolete_columns(cls, cursor, table_name: str, existing_columns: dict, schema: dict):
        """Removes columns that are no longer in the Pydantic models with logs."""
        for column in existing_columns:
            if column not in schema:
                cursor.execute(f'ALTER TABLE {table_name} DROP COLUMN {column} CASCADE;')                
                cls.log_change("DROP COLUMN", f"Obsolete column '{column}' removed in '{table_name}'")
    
    @classmethod
    def pydantic_to_sql(cls, model: type[BaseDBModel]) -> dict:
        """Converts a Pydantic model to a PostgreSQL schema with validations."""
        field_mapping = {
        ...
        for field_name, field in model.model_fields.items():
            ...
            is_unique = field.json_schema_extra.get("unique", False) if field.json_schema_extra else False
            primary_key = field.json_schema_extra.get("primary_key", False) if field.json_schema_extra else False
            foreign_key = field.json_schema_extra.get("foreign_key", None) if field.json_schema_extra else None
            track_changes = field.json_schema_extra.get("track_changes", False) if field.json_schema_extra else False
            default = field.default if field.default and not isinstance(field.default, PydanticUndefinedType) else False
            on_delete = field.json_schema_extra.get("on_delete", False) if field.json_schema_extra else False
            on_update = field.json_schema_extra.get("on_update", False) if field.json_schema_extra else False
    ...
    def generate_create_table_query(cls, table: str, schema: dict) -> tuple[str]:
        """Generates an SQL query to create a table with foreign keys and unique constraints."""
        columns = []
        constraints = []
        contraints_fk = []
        ...
        query = f"CREATE TABLE IF NOT EXISTS {table} ({', '.join(columns + constraints)});"
        return (query, "".join(contraints_fk))
    
    @classmethod
    def update_table_schema(cls, cursor, table_name: str, schema: dict):
        """Detects and applies changes in the table schema with logs."""
        ...
        for column, column_props in schema.items():
            ...
            if column not in existing_columns:
                ...
                cls.log_change("ADD COLUMN", f"Column '{column}' added to '{table_name}'")

                if is_required:
                    cursor.execute(f'ALTER TABLE {table_name} ALTER COLUMN {column} SET NOT NULL;')
                    cls.log_change("ALTER COLUMN", f"Column '{column}' in '{table_name}' marked as NOT NULL")

                if is_unique:
                    cursor.execute(f'ALTER TABLE {table_name} ADD CONSTRAINT {column}_unique UNIQUE ({column});')
                    cls.log_change("ADD CONSTRAINT", f"Column '{column}' in '{table_name}' is now UNIQUE")

                if foreign_key:
                    cls.add_foreign_key(cursor, table_name, column, foreign_key, on_delete, on_update)
                elif column in existing_foreign_keys:
                    cls.remove_foreign_key(cursor, table_name, column)

                if default:
                    cursor.execute(f"ALTER TABLE {table_name} ALTER COLUMN {column} SET DEFAULT '{default}'::{column_type};")
                    cls.log_change("ALTER COLUMN", f"Column '{column}' in '{table_name}' set default to '{default}'::{column_type}")
                
                if primary_key:
                    cursor.execute(f'ALTER TABLE {table_name} ADD PRIMARY KEY ({column});')
                    cls.log_change("ADD PRIMARY KEY", f"Primary key '{column}' added to '{table_name}'")
                ...
                if existing_columns[column].get("type") != column_type:
                    ...
                    cursor.execute(alter_query)
                    cls.log_change("ALTER COLUMN", f"Column '{column}' in '{table_name}' is now of type {column_type}")
                ...
                if not is_required and existing_columns[column].get("required"):
                    ...
                    cls.log_change("ALTER COLUMN", f"Column '{column}' in '{table_name}' marked as NULLABLE")

                if is_unique and not existing_columns[column].get("unique"):
                    # Drop if exists and add again
                    cursor.execute(f'ALTER TABLE {table_name} DROP CONSTRAINT IF EXISTS {column}_unique;')
                ...
                if not is_unique and existing_columns[column].get("unique"):
                    cursor.execute(f'ALTER TABLE {table_name} DROP CONSTRAINT IF EXISTS {column}_unique;')
                    cls.log_change("DROP CONSTRAINT", f"Column '{column}' in '{table_name}' is no longer UNIQUE")

                if foreign_key and existing_foreign_keys.get(column) != foreign_key:
                    cls.remove_foreign_key(cursor, table_name, column)
                    cls.add_foreign_key(cursor, table_name, column, foreign_key, on_delete, on_update)
                    cls.log_change("ALTER FOREIGN KEY", f"Foreign key '{column}' -> '{foreign_key}.id' in '{table_name}'")
                ...
                if default and existing_columns[column].get("default") != default:
                    cursor.execute(f"ALTER TABLE {table_name} ALTER COLUMN {column} SET DEFAULT '{default}'::{ existing_columns[column].get('type')};")
                    cls.log_change("ALTER COLUMN", f"Column '{column}' in '{table_name}' set to default '{default}'")           
    ...
    def enable_data_tracking(cls, cursor, table_name: str, schema: dict):
        """Enables data change tracking for fields with track_changes=True."""
        tracking_fields = [col for col, props in schema.items() if props.get("track_changes")]
        ...
        if tracking_fields:
            ...
            cls.log_change("ENABLE DATA TRACKING", f"Data change tracking enabled in '{table_name}' for {tracking_fields}")

    @classmethod
    def enable_unaccent_extension(cls, cursor):
        """Enables the 'unaccent' extension in the database."""
    ...
    def disable_removed_tracking_fields(cls, cursor, table_name: str, schema: dict):
        """Disables data tracking on removed or track_changes=False columns."""
        tracking_fields = [col for col, props in schema.items() if props.get("track_changes")]
        ...
        if tracking_fields:
            ...
            cls.log_change("DISABLE DATA TRACKING", f"Data tracking removed in '{table_name}'")
